import { useState, useRef, useCallback } from 'react';
import { useParams, useNavigate } from 'react-router-dom';
import { ArrowLeft, StickyNote, Info } from 'lucide-react';
import { machineStore, islandStore, noteStore, hallStore, machineModelStore } from '../store';
import { MachineNote, SettingRating } from '../types';

type Tab = 'memo' | 'info';

function StarRating({ value, onChange }: { value?: SettingRating; onChange: (v: SettingRating) => void }) {
  return (
    <div className="flex gap-1">
      {([1, 2, 3, 4, 5] as SettingRating[]).map(n => (
        <button
          key={n}
          className={`text-2xl ${n <= (value ?? 0) ? 'text-yellow-400' : 'text-gray-300'}`}
          onClick={() => onChange(value === n ? (1 as SettingRating) : n)}
        >
          ★
        </button>
      ))}
    </div>
  );
}

function ratingLabel(r?: SettingRating): string {
  if (!r) return '未評価';
  return ['', '入りにくい', 'やや入りにくい', '普通', 'やや入りやすい', '入りやすい'][r];
}

export default function MachineDetailPage() {
  const { hallId, machineId } = useParams<{ hallId: string; machineId: string }>();
  const navigate = useNavigate();

  const machine = machineStore.getById(machineId!);
  const island = machine ? islandStore.getAll().find(i => i.id === machine.islandId) : undefined;
  const hall = hallStore.getAll().find(h => h.id === hallId);
  const allModels = machineModelStore.getAll();

  const [tab, setTab] = useState<Tab>('memo');
  const [note, setNote] = useState<MachineNote | undefined>(() => noteStore.getByMachine(machineId!));

  // ─── メモタブ ───
  const [memoText, setMemoText] = useState(note?.memo ?? '');
  const [settingRating, setSettingRating] = useState<SettingRating | undefined>(note?.settingRating);
  const memoTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);

  const saveMemo = useCallback((text: string, rating: SettingRating | undefined) => {
    noteStore.upsert(machineId!, { settingRating: rating, memo: text });
    setNote(noteStore.getByMachine(machineId!));
  }, [machineId]);

  function handleMemoChange(text: string) {
    setMemoText(text);
    if (memoTimerRef.current) clearTimeout(memoTimerRef.current);
    memoTimerRef.current = setTimeout(() => saveMemo(text, settingRating), 800);
  }

  function handleRatingChange(rating: SettingRating) {
    setSettingRating(rating);
    if (memoTimerRef.current) clearTimeout(memoTimerRef.current);
    saveMemo(memoText, rating);
  }

  // ─── 台情報タブ ───
  const [machineModelId, setMachineModelId] = useState(machine?.machineModelId ?? '');
  const [machineNumber, setMachineNumber] = useState(machine?.number ?? '');
  const infoTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);

  function saveInfo(number: string, modelId: string) {
    if (!machine) return;
    const selected = allModels.find(m => m.id === modelId);
    machineStore.update(machine.id, {
      number,
      machineModelId: modelId || undefined,
      modelName: selected?.name ?? '',
    });
  }

  function handleNumberChange(number: string) {
    setMachineNumber(number);
    if (infoTimerRef.current) clearTimeout(infoTimerRef.current);
    infoTimerRef.current = setTimeout(() => saveInfo(number, machineModelId), 800);
  }

  function handleModelChange(modelId: string) {
    setMachineModelId(modelId);
    saveInfo(machineNumber, modelId);
  }

  if (!machine || !island) {
    return (
      <div className="flex items-center justify-center h-full">
        <p className="text-gray-500">台が見つかりません</p>
      </div>
    );
  }

  const resolvedModelName = (() => {
    const m = machineModelId ? allModels.find(m => m.id === machineModelId) : null;
    return m?.name ?? machine.modelName;
  })();

  return (
    <div className="flex flex-col h-full bg-gray-100">
      {/* ヘッダー */}
      <header className="bg-blue-800 text-white px-4 py-3 flex items-center gap-3 shadow">
        <button onClick={() => navigate(`/halls/${hallId}`)} className="active:opacity-70">
          <ArrowLeft size={22} />
        </button>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <h1 className="text-lg font-bold">{machineNumber}番台</h1>
            {note?.settingRating && (
              <span className="text-yellow-300 text-sm">{'★'.repeat(note.settingRating)}</span>
            )}
          </div>
          <p className="text-xs text-blue-200 truncate">
            {hall?.name} / {island.name}
            {resolvedModelName ? ` / ${resolvedModelName}` : ''}
          </p>
        </div>
      </header>

      {/* タブ */}
      <div className="flex bg-white border-b border-gray-200 shadow-sm">
        {([
          { key: 'memo', label: 'メモ', icon: StickyNote },
          { key: 'info', label: '台情報', icon: Info },
        ] as { key: Tab; label: string; icon: React.ElementType }[]).map(({ key, label, icon: Icon }) => (
          <button
            key={key}
            className={`flex-1 py-2.5 text-sm font-medium flex items-center justify-center gap-1.5 transition-colors ${
              tab === key ? 'text-blue-700 border-b-2 border-blue-700' : 'text-gray-500'
            }`}
            onClick={() => setTab(key)}
          >
            <Icon size={14} />
            {label}
          </button>
        ))}
      </div>

      {/* コンテンツ */}
      <div className="flex-1 overflow-y-auto">
        {/* ─── メモタブ ─── */}
        {tab === 'memo' && (
          <div className="p-4 flex flex-col gap-4">
            <div className="bg-white rounded-xl p-4 shadow">
              <p className="text-sm font-semibold text-gray-700 mb-2">設定の入りやすさ</p>
              <StarRating value={settingRating} onChange={handleRatingChange} />
              <p className="text-xs text-gray-400 mt-1">{ratingLabel(settingRating)}</p>
            </div>

            <div className="bg-white rounded-xl p-4 shadow">
              <p className="text-sm font-semibold text-gray-700 mb-2">メモ</p>
              <textarea
                className="w-full border border-gray-200 rounded-lg px-3 py-2 text-base outline-none focus:border-blue-400 resize-none"
                rows={6}
                placeholder="台の特徴、傾向、気になった点など自由に..."
                value={memoText}
                onChange={e => handleMemoChange(e.target.value)}
              />
            </div>
          </div>
        )}

        {/* ─── 台情報タブ ─── */}
        {tab === 'info' && (
          <div className="p-4 flex flex-col gap-4">
            <div className="bg-white rounded-xl p-4 shadow flex flex-col gap-3">
              <div>
                <label className="text-sm font-medium text-gray-600">台番号</label>
                <input
                  className="mt-1 w-full border border-gray-300 rounded-lg px-3 py-2 text-base outline-none focus:border-blue-500"
                  value={machineNumber}
                  onChange={e => handleNumberChange(e.target.value)}
                />
              </div>
              <div>
                <label className="text-sm font-medium text-gray-600">機種名（機種DB参照）</label>
                <select
                  className="mt-1 w-full border border-gray-300 rounded-lg px-3 py-2 text-base outline-none bg-white focus:border-blue-500"
                  value={machineModelId}
                  onChange={e => handleModelChange(e.target.value)}
                >
                  <option value="">未設定</option>
                  {allModels.map(m => (
                    <option key={m.id} value={m.id}>{m.name}</option>
                  ))}
                </select>
                {allModels.length === 0 && (
                  <p className="text-xs text-gray-400 mt-1">機種DBに機種を登録すると選択できます</p>
                )}
              </div>
            </div>

            <div className="bg-white rounded-xl p-4 shadow">
              <p className="text-xs text-gray-500">ホール</p>
              <p className="text-base text-gray-800">{hall?.name}</p>
              <p className="text-xs text-gray-500 mt-2">機種タイプ</p>
              <p className="text-base text-gray-800">
                {island.machineType === 'pachinko' ? 'パチンコ' : 'スロット'}
              </p>
            </div>
          </div>
        )}
      </div>
    </div>
  );
}
